    ...
    <div className="relative min-h-[80vh] p-6 text-text-primary">
      ...
      <div className="relative z-10">
        ...
        <div className="grid gap-6 md:grid-cols-5 mb-8">
          <StatBox label="Total PO" value={m.totalPO} icon={<FileText size={20} className="text-white/60" />} />
        ...
        <div className="rounded-2xl border border-white/5 bg-background-secondary/40 backdrop-blur-xl p-6 shadow-2xl">
          <h2 className="text-lg font-bold tracking-wide text-white mb-6">PURCHASE ORDER LIST</h2>
                ...
                {orders.length === 0 ? (
                  ...
                  orders.map((order: any, idx: number) => (
                    <tr key={idx} className="transition-colors hover:bg-white/5">
                      <td className="px-6 py-4 font-mono font-medium text-white/80">{order.id}</td>
                      <td className="px-6 py-4 font-semibold text-white">{order.supplierName}</td>
...
function StatBox({ label, value, icon, color = "text-white" }: { label: string; value: number; icon: React.ReactNode; color?: string; }) {
  return (
