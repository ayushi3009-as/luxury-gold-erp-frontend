...
export default function SilverRatePage() {
  ...
  const currentRate = history[0] || { silver: 92500, createdAt: new Date() };
  const prevRate = history[1];

  const pricePerKg = currentRate.silver;
  const pricePer10g = pricePerKg / 100;
  ...
  if (prevRate) {
    const prevKg = prevRate.silver;
    prevPricePerGram = prevKg / 1000;
  ...
  // Calculate highest/lowest today (using current vs previous for demo purposes as we don't have intraday data yet)
  ...
  return (
    <div className="min-h-screen bg-background-primary text-text-primary">
      <main className="p-8">
        {/* Header */}
        <div className="mb-8">
          <p className="text-sm text-text-secondary">Gold Rate / Silver Rate</p>
          <div className="mt-2 flex items-center justify-between">
            <div>
              <h1 className="text-3xl font-bold">Silver Rate</h1>
              <p className="mt-2 text-text-secondary">Monitor current silver market rates and price movement.</p>
            </div>
        ...
        <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-4">
          ...
          <div className="rounded-xl border border-border-theme bg-background-secondary p-5">
            <div className="flex items-center justify-between">
              <Coins size={26} className="text-accent-gold" />
            ...
            <p className="mt-5 text-sm text-text-secondary">SILVER RATE / GRAM</p>
            <h2 className="mt-2 text-2xl font-bold">₹ {pricePerGram.toLocaleString("en-IN", { minimumFractionDigits: 2 })}</h2>
          </div>

          {/* 10 Gram */}
          <div className="rounded-xl border border-border-theme bg-background-secondary p-5">
            <p className="text-sm text-text-secondary">SILVER RATE / 10 GRAM</p>
          ...
          {/* 1 KG */}
          <div className="rounded-xl border border-border-theme bg-background-secondary p-5">
            <p className="text-sm text-text-secondary">SILVER RATE / KG</p>
            <h2 className="mt-5 text-2xl font-bold">₹ {pricePerKg.toLocaleString("en-IN", { minimumFractionDigits: 2 })}</h2>
        ...
        <div className="mt-6 rounded-xl border border-border-theme bg-background-secondary p-6">
          <div className="flex items-center justify-between">
            <h2 className="text-lg font-semibold text-accent-gold">SILVER RATE DETAILS</h2>
          ...
          <div className="mt-6 grid gap-5 md:grid-cols-3">
            <div className="rounded-lg border border-[#302a1b] bg-[#151611] p-4">
              ...
              <p className="mt-2 text-lg font-semibold">999 Fine Silver</p>
            </div>
            <div className="rounded-lg border border-[#302a1b] bg-[#151611] p-4">
        ...
        <div className="mt-6 grid gap-5 xl:grid-cols-2">
          ...
          <div className="rounded-xl border border-border-theme bg-background-secondary p-6">
            ...
            <div className="mt-5 grid gap-3">
              ...
              <Link href="/gold-rate/rate-comparison" className="rounded-lg border border-[#4a3a18] bg-[#151611] p-3 text-left text-sm hover:bg-[#211c0e] transition-colors block text-text-primary text-decoration-none">
                Compare With Gold
              </Link>
            </div>
          </div>
