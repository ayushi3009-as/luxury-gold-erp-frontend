...
export default async function AboutPage({ params }: { params: { domain: string } }) {
  ...
  return (
    <div className="pt-32 pb-24 px-6 max-w-4xl mx-auto min-h-screen text-center">
      <p className="text-accent-gold uppercase tracking-[0.3em] text-sm font-semibold mb-6">Our Heritage</p>
      <h1 className="text-5xl md:text-6xl font-serif text-white mb-12 leading-tight">
        A Legacy of <br /> Flawless Craftsmanship
      </h1>

      <div className="w-full aspect-video bg-[#111] mb-16 overflow-hidden">
        <img 
      ...
      <div className="space-y-8 text-lg text-white/70 font-light leading-relaxed text-left">
        <p>
          {tenant?.aboutUsText || 
      ...
      <div className="mt-24 grid grid-cols-1 md:grid-cols-3 gap-12 border-t border-white/10 pt-16">
        <div>
          <h3 className="text-2xl font-serif text-white mb-4">Purity</h3>
          <p className="text-white/50 text-sm">Certified 22K and 24K gold, verified for authenticity.</p>
        </div>
        <div>
          <h3 className="text-2xl font-serif text-white mb-4">Precision</h3>
          <p className="text-white/50 text-sm">Flawless diamonds cut to maximize brilliance and fire.</p>
        </div>
        <div>
          <h3 className="text-2xl font-serif text-white mb-4">Passion</h3>
          <p className="text-white/50 text-sm">Handcrafted by master artisans with decades of experience.</p>
        </div>
      </div>
    </div>
  );
