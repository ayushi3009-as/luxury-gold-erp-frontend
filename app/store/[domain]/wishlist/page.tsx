  ...
  return (
    <div className="pt-32 pb-24 px-6 max-w-7xl mx-auto min-h-screen">
      <div className="border-b border-white/10 pb-8 mb-12">
        ...
        <h1 className="text-4xl md:text-5xl font-serif text-white">Wishlist</h1>
      </div>
        ...
        {dummyWishlist.map((item) => (
          <div key={item.id} className="group flex flex-col">
            <Link href={`/product/${item.id}`} className="relative aspect-[4/5] overflow-hidden bg-[#111] mb-4">
              ...
              <button className="absolute top-4 right-4 p-2 bg-black/50 text-white rounded-full hover:bg-red-500/80 transition-colors backdrop-blur-sm z-10">
                <Trash2 size={16} />
              </button>
            </Link>
            <Link href={`/product/${item.id}`} className="flex-grow">
              <h4 className="text-sm font-serif text-white/90 group-hover:text-gold transition-colors">{item.name}</h4>
              <p className="text-xs text-white/50 my-1">{item.purity}</p>
