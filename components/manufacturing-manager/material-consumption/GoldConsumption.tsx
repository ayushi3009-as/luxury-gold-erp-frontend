...
export default function GoldConsumption() {
  ...
  const fetchRecords = async () => {
    try {
      ...
      // Filter by material name containing "Gold" (case insensitive)
      const filtered = data.filter((d: any) => d.materialName?.toLowerCase().includes("gold"));
      setRecords(filtered);
    } catch (error) {
    ...
    <div className="rounded-2xl border border-border-theme bg-background-secondary">
      <div className="flex flex-col gap-4 border-b border-border-theme p-6 md:flex-row md:items-center md:justify-between">
        <div className="flex items-center gap-3">
          <h2 className="text-xl font-semibold text-text-primary">Gold Consumption</h2>
