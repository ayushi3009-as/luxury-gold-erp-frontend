...
import { Search, Eye, Pencil, Trash2, Coins } from "lucide-react";
...
export default function SilverConsumption() {
  ...
  const fetchRecords = async () => {
    try {
      ...
      // Filter by material name containing "Silver" (case insensitive)
      const filtered = data.filter((d: any) => d.materialName?.toLowerCase().includes("silver"));
      setRecords(filtered);
    } catch (error) {
    ...
    <div className="rounded-2xl border border-border-theme bg-background-secondary">
      <div className="flex flex-col gap-4 border-b border-border-theme p-6 md:flex-row md:items-center md:justify-between">
        <div className="flex items-center gap-3">
          <h2 className="text-xl font-semibold text-text-primary">Silver Consumption</h2>
